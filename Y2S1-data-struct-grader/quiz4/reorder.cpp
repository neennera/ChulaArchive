...
namespace CP
{
  ...
  public:
    ...
    void reorder(int pos, std::vector<int> selected)
    {
      ...
      auto it2 = mHeader->next;
      auto in = mHeader->next;
      while (pos--)
      {
        in = in->next;
      }
      ...
      in->prev->next = temp.mHeader->next;
      temp.mHeader->next->prev = in->prev;
      temp.mHeader->prev->next = in;
      in->prev = temp.mHeader->prev;
      temp.mSize = 0;
    }
  };
