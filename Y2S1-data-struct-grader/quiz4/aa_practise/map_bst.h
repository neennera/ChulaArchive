...
namespace CP
{
  ...
  public:
    ...
    {
      KeyT ans_k = mRoot->data.first;
      int ans_h = -1;
      solve(mRoot, ans_k, ans_h);
      return ans_k;
    }

    int solve(node *n, KeyT &ans_k, int &ans_h)
    {
      if (n == NULL)
        return -1;
      int lh = n->left == NULL ? -1 : solve(n->left, ans_k, ans_h);
      int rh = n->right == NULL ? -1 : solve(n->right, ans_k, ans_h);
      KeyT now_k = n->data.first;
      // std::cout << now_k << " : l->" << lh << "  r:" << rh << "\n";
      if (ans_h == abs(lh - rh) && mLess(now_k, ans_k))
      {
        ans_k = now_k;
      }
      if (abs(lh - rh) > ans_h)
