...
namespace CP
{
  ...
  public:
    ...
    void reorder(int pos, std::vector<int> selected)
    {
      ...
      if (selected.empty())
        return;
      node *in = mHeader->next;
      while (pos--)
      {
        in = in->next;
      }

      // colect inserted node
      CP::list<std::string> l;
      int ct = 0;
      auto it = selected.begin();  // value in selected
      iterator tt = mHeader->next; // iterator in normal list
      for (int i = 0; i < mSize; i++)
      {
        ...
        if (*it == i)
        {
          ...
          l.push_back(tt.ptr->data);
          tt.ptr->prev->next = tt.ptr->next;
          tt.ptr->next->prev = tt.ptr->prev;
          it++;
        }
      ...
      in->prev->next = l.mHeader->next;
      l.mHeader->next->prev = in->prev;
      l.mHeader->prev->next = in;
      in->prev = l.mHeader->prev;
      l.mSize = 0;
      // std::cout << "DONE reorder\n";
    }
  };
